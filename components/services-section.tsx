...
import { Scale, Building2, Wallet, SearchCheck, Handshake, Leaf, Landmark, Users, Calculator, Target, Globe, ChevronRight, CircleCheck as CheckCircle2 } from 'lucide-react';
...
export function ServicesSection({ servicesData }: ServicesSectionProps) {
  ...
  return (
    <section id="services" className="py-32 bg-gradient-to-b from-white via-blue-50/20 to-white relative overflow-hidden">
        ...
        <AnimatedSection>
          <div className="max-w-3xl mx-auto text-center mb-20">
            ...
            <p className="text-xl font-body text-muted-foreground leading-relaxed text-pretty">
              От правового сопровождения до управления проектами —
              ...
              <AnimatedSection key={service.id} delay={index * 0.05}>
                ...
                >
                  <div className="group relative bg-white/95 backdrop-blur-sm rounded-3xl p-8 border-2 border-gray-100 hover:border-transparent hover:shadow-2xl transition-all duration-500 overflow-hidden">
                    ...
                    <div className={`absolute top-0 left-0 w-2 h-full bg-gradient-to-b ${gradient} transform origin-left transition-transform duration-500 ${isHovered ? 'scale-x-100' : 'scale-x-0'}`} />

                    <div className="flex flex-col md:flex-row relative z-10  md:items-center gap-8">
                      <div className={`relative w-24 h-24 rounded-3xl bg-gradient-to-br ${gradient} flex items-center justify-center flex-shrink-0 shadow-2xl group-hover:scale-110 group-hover:rotate-12 transition-all duration-500`}>
                        <div className="absolute inset-0 rounded-3xl bg-white/20 backdrop-blur-sm" />
                        {Icon && <Icon className="h-12 w-12 text-white relative z-10" strokeWidth={1.5} />}
                        <div className={`absolute -inset-3 bg-gradient-to-br ${gradient} rounded-3xl blur-2xl opacity-40 group-hover:opacity-70 transition-opacity duration-500`} />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <motion.div
                            ...
                            transition={{ duration: 0.3 }}
                          >
                            <ChevronRight className={`h-8 w-8 text-gray-400 group-hover:text-primary transition-colors duration-300`} />
                          </motion.div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {service.benefits.slice(0, 4).map((benefit, idx) => (
                            <span key={idx} className="px-4 py-2 rounded-full text-sm font-medium bg-blue-50 text-blue-700 border border-blue-200">
                              {benefit}
                            </span>
                          ))}
                ...
                <DialogHeader>
                  <div className="flex items-center gap-4 mb-4">
                    {(() => {
                      ...
                      const gradient = gradientColors[services.indexOf(selectedService) % gradientColors.length];
                      return (
                    ...
                    {selectedService.benefits.map((benefit, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex items-start gap-3 p-4 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors">
                          <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${gradient} flex items-center justify-center flex-shrink-0 shadow-md`}>
                            <CheckCircle2 className="h-5 w-5 text-white" />
                          </div>
                          <span className="text-base text-foreground leading-relaxed pt-1">{benefit}</span>
                        </div>
                      );
                    })}
                  </div>
